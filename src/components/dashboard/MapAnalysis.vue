          ...
          <div class="drone-params">
            ...
            <div class="param-group">
              ...
              <div v-if="gridBlocksValid" class="grid-info">
                将划分为 {{ parsedGridBlocks.m }}×{{ parsedGridBlocks.n }} 个区块，共 {{ parsedGridBlocks.m * parsedGridBlocks.n }} 个巡逻区块
              </div>
            </div>
          </div>
      ...
      <div class="right-panel">
              ...
              <div v-if="selectedImage" class="result-display">
                ...
                <div class="image-container" ref="imageContainer" @mousemove="handleMouseMove">
                  ...
                  <div v-if="showGrid" class="grid-overlay" :style="gridOverlayStyle">
                    ...
                    >
                      <div class="block-label" v-if="selectedBlock && selectedBlock.id === block.id">
                        {{ block.row }}, {{ block.col }}
                      </div>
                    </div>
                  </div>
        ...
        <div class="threat-points-card">
          <div class="threat-header">
            ...
            <span v-else class="threat-count no-data">
              等待分析结果
            </span>
          </div>
          ...
          <div v-if="filteredThreatPoints.length > 0" class="threat-points-table">
            <table>
              ...
              <tbody>
                <tr v-for="point in filteredThreatPoints" :key="point.id" 
                  ...
                  <td class="location-cell">
                    <div class="location-info">
                      <div class="location-main">{{ point.location }}</div>
                      <div v-if="point.description" class="location-desc">{{ point.description }}</div>
                    </div>
                  </td>
...
const getBlockColor = (block) => {
  if (selectedBlockIndex.value !== null && gridBlocks.value[selectedBlockIndex.value]?.id === block.id) {
    return 'rgba(74, 108, 247, 0.3)'
  }
  if (highlightedBlockIndex.value !== null && gridBlocks.value[highlightedBlockIndex.value]?.id === block.id) {
...
const calculateImageDisplayArea = () => {
  if (!resultImage.value || !imageContainer.value) return
...
const handleMouseMove = (event) => {
  ...
  // 确保鼠标在图片区域内
  if (relativeX >= 0 && relativeX <= imageDisplayArea.value.width && 
      relativeY >= 0 && relativeY <= imageDisplayArea.value.height) {
    // 将容器坐标转换为图片坐标
    const imgX = Math.round(relativeX * (imageNaturalSize.value.width / imageDisplayArea.value.width))
    const imgY = Math.round(relativeY * (imageNaturalSize.value.height / imageDisplayArea.value.height))
...
const startAnalysis = async () => {
  ...
  try {
    ...
    // 生成模拟威胁点数据
    const threatPoints = generateThreatPoints()
    
    // 模拟分析结果
    analysisResult.value = {
      heatmap: "/pictures/屏幕截图 2026-04-17 190748.png",
      path: "/pictures/屏幕截图 2026-04-17 190659.png",
      stats: {
        pathLength: '1245.6',
        maxThreat: '81.5',
        avgThreat: '24.2',
      ...
      threatStats: {
        topThreatBlock: '4-1',
        distribution: '主要分布在左下和道路沿线',
        priorityPatrol: '区块4-1、3-2、2-3、4-3'
      }
    }
...
.image-container {
  ...
  cursor: crosshair;
  padding: 0rem;
  position: relative; /* 新增：为内部绝对定位元素建立参考 */
}
...
.base-image, .overlay-image {
  ...
  max-height: 100%;
  object-fit: contain;
  pointer-events: none; /* 防止叠加层干扰鼠标事件 */
}
...
.grid-overlay {
  ...
  top: 1rem; /* 与容器的padding一致 */
  left: 1rem; /* 与容器的padding一致 */
  width: calc(100% - 2rem); /* 减去左右padding */
  height: calc(100% - 2rem); /* 减去上下padding */
  pointer-events: none; /* 容器本身不拦截鼠标事件 */
  z-index: 10;
}
