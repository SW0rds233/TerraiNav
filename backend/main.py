...
优化记录：
- 移除硬编码 API Key
- 所有 emoji 替换为 ASCII 标记，避免 Windows 控制台编码错误
"""
...
def main():
    ...
    if not api_key:
        ...
        print("请设置: set DASHSCOPE_API_KEY=your-key  (Windows)")
        print("        export DASHSCOPE_API_KEY=your-key  (Linux/Mac)")
        return 1
    ...
    try:
        ...
        print(f"[FILE] 使用图片: {image_path}")
        print()

        # 1. 初始化分析器
        print("=" * 70)
        print("[Step 1] 初始化AI分析器")
        print("=" * 70)
        ...
        print()
        print(f"[OK] 地形识别完成, 共识别到 {len(terrain_data)} 个地形要素\n")

        # 3. 威胁评估
        print("=" * 70)
        print("[Step 3] FAHP威胁度评估")
        print("=" * 70)
        ...
        print(
            f"[OK] 威胁矩阵构建完成, 尺寸: {threat_matrix.shape[0]} x {threat_matrix.shape[1]}\n"
        )

        # 5. 路径规划
        print()
        print("=" * 70)
        print("[Step 5] ACO蚁群路径规划")
        print("=" * 70)
        planner = PathPlanner()
        ...
        print(f"[OK] 威胁矩阵已保存到: {matrix_file}")
        ...
        print(f"[OK] 热力图已保存到: {heatmap_file}")
        ...
        print(f"[OK] 分析结果已保存到: {result_file}")
        ...
        print()
        print("生成的文件:")
        print(f"  [CSV] {matrix_file}  - 威胁矩阵CSV")
        print(f"  [MAP] {heatmap_file}  - 威胁热力图")
        print(f"  [JSON] {result_file} - 完整分析结果JSON")
        print()
